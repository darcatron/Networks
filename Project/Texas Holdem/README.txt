...
———————————————————————————————————————
    ...
    -There is no way shut down the server without losing all the user data since it is saved
        in program memory. A potential fix for this is to have a key press that runs a 
        mimic_down function and rejects all requests to the server. The data is then saved
        once the mimic_down is disabled.
